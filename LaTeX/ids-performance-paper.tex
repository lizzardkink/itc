...
\begin{abstract}
This study quantifies the computational overhead of Intrusion Detection/Prevention Systems (IDS/IPS) on Windows 11 by benchmarking TotalAV antivirus (v6.5.219) and Fort Firewall (v3.19.9). We evaluated four configurations: baseline (no IDS/IPS), TotalAV-only, Fort Firewall-only, and combined deployment. Six performance metrics were measured across five iterations: boot time, RAM consumption, process count, application launch, local network throughput (SMB), and remote download speed (FTP). TotalAV imposed the highest overhead with 31.39\% increased boot time, 17.44\% higher RAM usage, and 50.05\% slower application launches. Fort Firewall improved boot time by 14.62\% and application launch by 18.05\%. Network performance degraded 8.80--20.02\% across all IDS/IPS configurations. Our results indicate that while IDS/IPS software provides essential security, careful consideration of performance trade-offs is necessary for resource-constrained environments.
...
This research provides empirically-derived performance metrics for contemporary IDS/IPS solutions on Windows~11.
Our methodology follows standardized testing practices including controlled environments, multiple iterations (3--5 runs), IQR outlier detection, and baseline comparisons~\cite{tomshardware-methodology,tomshardware-testing-guide,anandtech-benchmarks,anandtech-statistical-methods}.
Unlike vendor benchmarks showing best-case scenarios, this evaluation enables data-driven security decisions.

\section{Related Work}
\label{sec:related}

Computer performance evaluation follows industry standards including controlled environments, multiple iterations (3--5 runs), and IQR outlier detection~\cite{tomshardware-methodology,tomshardware-testing-guide,anandtech-benchmarks,anandtech-statistical-methods}.
Our methodology adheres to these principles.
...
\begin{enumerate}
\item \textbf{Baseline:} Clean Windows 11 Home (25H2 build 26200), default settings, Defender and Firewall active
\item \textbf{TotalAV Only:} TotalAV v6.5.219 active
\item \textbf{Fort Firewall Only:} Fort Firewall v3.19.9 active
...
\textbf{Criterion D -- Application Launch:} Custom PowerShell script launching 30 applications (10 Calculator, 10 Paint, 10 Notepad) per iteration in randomized order to circumvent any pre-caching mechanism, measuring total time. 5 iterations across 5 boot cycles per configuration (25 measurements, 750 total app launches).

\textbf{Criterion E -- SMB Network Transfer:} Server Message Block protocol copying 1.01~GB folder from QNAP NAS (192.168.50.99) over 1~Gbps LAN, measuring transfer speed (MB/s). 5 iterations across 5 boot cycles per configuration (25 measurements).

\textbf{Criterion F -- FTP Remote Download:} FTP download of 114.66~MB file from DIGI Storage, measuring download speed (MB/s). 5 iterations across 5 boot cycles per configuration (25 measurements).
...
Outlier removal resulted in varying sample sizes across metrics (3--30 measurements per configuration after cleaning). All reported statistics reflect cleaned datasets.
...
\caption{Application Launch Performance for 30 Applications. TotalAV imposes most severe overhead (+50.05\%), increasing launch time by half. Fort Firewall shows unexpected 18.05\% improvement.}
...
\textbf{Superadditive Effects:} Boot time overhead (24.09\%) exceeded the expected additive sum of individual components (31.39\% + (-14.62\%) = 16.77\%), indicating negative interaction where TotalAV's scanning overhead partially negates Fort Firewall's optimization benefits when both run simultaneously.

\textbf{Subadditive Effects:} App launch overhead (31.21\% vs.\ expected 32.00\%) showed slightly less degradation than the sum would predict, suggesting some shared Windows security APIs reduce redundant system calls.

\textbf{Near-Additive Effects:} RAM usage (20.35\% vs.\ expected 18.92\%) and FTP performance (-20.02\% vs.\ -28.24\%) remained largely independent, with both systems maintaining separate memory footprints and network inspection overhead.
...
Organizations must weigh security needs against performance through careful product selection and deployment strategies.
The quantitative metrics provided enable data-driven security policy development.
Fort Firewall's performance improvements suggest optimization opportunities in mainstream security products.
